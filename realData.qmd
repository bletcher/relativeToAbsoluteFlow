...
get_figures <- function(dIn, dInSampleApprox, dInSample) {
  fig_flow <- ggplot(dIn, aes(score_scaled, score_scaled_ecdf)) + 
    ...
    labs(x = "Scaled observed flow", y = "Empirical cumulative distribution") +
    theme_publication() 
  
  fig_score <- ggplot(dIn, aes(score_scaled, score_scaled_ecdf)) + 
    geom_line(size = 2) +
    labs(x = "Scaled image score", y = "Empirical cumulative distribution") +
    theme_publication() 
  ...
  fig_subsample <- ggplot(dIn) + 
    ...
    labs(x = "Blue = observed scaled flow, Orange = subsampled flow", y = "Empirical cumulative distribution") +
    theme_publication() 
  ...
  fig_smooth_binomial <- ggplot(dIn, aes(score_scaled, score_scaled_ecdf)) + 
    ...
    labs(x = "Black = scaled image score, Orange = subsampled flow", y = "Empirical cumulative distribution") +
  ...
  return(list(
    fig_flow=fig_flow, fig_score=fig_score, fig_subsample=fig_subsample, fig_smooth_binomial=fig_smooth_binomial, fig_smooth_gam=fig_smooth_gam, fig_smooth_interpolate=fig_smooth_interpolate)
    )
}

plotSubSample <- function(dIn0, numSample, add_anchors = TRUE){
  ...
  dInSampleApprox <- data.frame(approx(dInSample$flow_cfs_log10_scaled, dInSample$flow_cfs_log10_scaled_ecdf, dInSample$flow_cfs_log10_scaled))
  
  figs <- get_figures(dIn, dInSampleApprox, dInSample)
  
  
  ggarrange(figs$fig_flow, figs$fig_score, figs$fig_subsample, figs$fig_smooth_interpolate, figs$fig_smooth_binomial, figs$fig_smooth_gam, ncol = 2, nrow = 3)
...
plotSubSample <- function(dIn0, numSample){
  ...
  get_figs_no_anchors <- get_figures(dIn, dInSampleApprox_no_anchors, dInSample_no_anchors)
  
  figs_no_anchors <- ggarrange(
    get_figs_no_anchors$fig_flow, 
    get_figs_no_anchors$fig_score, 
    get_figs_no_anchors$fig_subsample, 
    get_figs_no_anchors$fig_smooth_interpolate, 
    get_figs_no_anchors$fig_smooth_binomial, 
    get_figs_no_anchors$fig_smooth_gam, 
    ncol = 2, nrow = 3
  )
  ...
  get_figs_with_anchors <- get_figures(dIn, dInSampleApprox_with_anchors, dInSample_with_anchors)
  
  figs_with_anchors <- ggarrange(
    get_figs_with_anchors$fig_flow, 
    get_figs_with_anchors$fig_score, 
    get_figs_with_anchors$fig_subsample, 
    get_figs_with_anchors$fig_smooth_interpolate, 
    get_figs_with_anchors$fig_smooth_binomial, 
    get_figs_with_anchors$fig_smooth_gam, 
    ncol = 2, nrow = 3
  )
  ...
  return(
    list(
      figs_no_anchors=figs_no_anchors, 
      figs_with_anchors=figs_with_anchors
    )
  )
...
100 samples  
First set of graphs includes min/max flow anchors, second set does not.
```{r}
