...
# get_preds_logistic <- function(dIn, num_pred_points = 100) {
#   mod <- glm(flow_cfs_log10_scaled_ecdf ~ flow_cfs_log10_scaled, family="binomial", data = dIn)
#   
#   x <- seq(min(dIn$flow_cfs_log10_scaled), max(dIn$flow_cfs_log10_scaled), length.out = num_pred_points)
#   df <- data.frame(
#     x = x,
#     y = predict(mod, data.frame(flow_cfs_log10_scaled = x),  type = "response")
#   )
#   return(df)
# }
...
get_smoothed_data_figs <- function(dIn, dInSampleApprox, dInSample, base_size_in = 12) {
  ...
  fig_smooth_gam <- ggplot(dIn, aes(score_scaled, score_scaled_ecdf)) + 
    ...
    geom_text(
      ...
      label = paste0(
        "Difference in area = ", 
        round(getECDF_diff(dIn$score_scaled_ecdf, get_preds_gam(dInSample)$y), 5)
      ),
      size = 8, 
      color = "orange"
      ) +
  ...
  fig_smooth_interpolate <- ggplot(dIn, aes(score_scaled, score_scaled_ecdf)) + 
    ...
    geom_text(
      aes(x = -1.5, y = 0.9), 
      label = paste0("Difference in area = ", 
      round(getECDF_diff(dIn$score_scaled_ecdf, dInSampleApprox$y), 5)), 
      size = 8, 
      color = "orange"
      ) +
...
plotSubSample <- function(dIn0, numSample, num_pred_points = 100, base_size_in = 12) {
  ...
  #     xout = seq(min(dInSample_with_anchors$flow_cfs_log10_scaled), max(dInSample_with_anchors$flow_cfs_log10_scaled), length.out = num_pred_points)
  #     #dInSample_with_anchors$flow_cfs_log10_scaled
  #     )
  #   )
  # 
  ...
  get_smoothed_data_figs_with_anchors <- get_smoothed_data_figs(dIn, dInSample, dInSample_interp_with_anchors, dInSample_gam_with_anchors, base_size = base_size_in)
...
dummy <- 2
plotSubSample(dWB0, 100, base_size_in = 10)
...
dummy <- 2
plotSubSample(dWB0, 10, base_size_in = 8)
...
dummy <- 2
plotSubSample(dAB, 100, base_size_in = 10)
...
dummy <- 2
plotSubSample(dAB, 10, base_size_in = 8)
