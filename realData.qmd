...
#dWB0$flow_cfs_log10_scaled_ecdf = getECDF(dWB0$flow_cfs_log10_scaled)
...
plotSubSample <- function(dIn0, numSample, add_anchors = TRUE){
  ...
  dInSampleApprox <- data.frame(approx(dIn$score_scaled, dIn$score_scaled_ecdf, dInSample$score_scaled))
  
  ggplot(dIn, aes(score_scaled, score_scaled_ecdf)) + 
    #stat_ecdf(geom = "step") +
    #stat_ecdf(aes(flow_cfs_log10_scaled), color = "blue") +
    geom_line() +
    geom_line(aes(flow_cfs_log10_scaled, flow_cfs_log10_scaled_ecdf), color = "blue") +
    geom_point(aes(flow_cfs_log10_scaled, flow_cfs_log10_scaled_ecdf), color = "orange", size = 4, data = dInSample) +
    #geom_point(aes(x,y), data = dInSampleApprox) +
    ...
    labs(x = "Scaled values, blue = Observed flow, black = image score, orange = subsampled flow", y = "Empirical cumulative distribution") +
    theme_publication() 
}

plotSubSample(dWB0, 100)
plotSubSample(dWB0, 100, FALSE)

plotSubSample(dWB0, 10)
plotSubSample(dWB0, 10, FALSE)

plotSubSample(dWB0, 1)
plotSubSample(dWB0, 1, FALSE)
